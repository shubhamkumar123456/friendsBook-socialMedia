...
const Friends = () => {
  ...
  useEffect(()=>{
    const fetchUserAllPost = async()=>{
      let res = await fetch(`http://localhost:3001/api/posts/getsinglePost/${location.state._id}`);
  ...
  const handleFollowClick =async()=>{
    let res  = await fetch(`http://localhost:3001/api/users/follow/${user._id}`,{
      method:'POST',
      headers:{
  ...
  const handleUnFollowClick =async()=>{
    let res  = await fetch(`http://localhost:3001/api/users/unfollow/${user._id}`,{
      method:'POST',
      headers:{
        'content-type': 'application/json'
      },
